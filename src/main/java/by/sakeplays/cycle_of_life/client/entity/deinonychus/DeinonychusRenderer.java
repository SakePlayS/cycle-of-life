...
public class DeinonychusRenderer extends GeoEntityRenderer<Deinonychus>  {
    ...
    public void postRender(PoseStack poseStack, Deinonychus animatable, BakedGeoModel model, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
        ...
        if (tick != animatable.getPlayer().tickCount) tick(animatable, model);
    ...
    private void tick(Deinonychus animatable, BakedGeoModel model) {
        ...
        if (animatable.getPlayer() == Minecraft.getInstance().player) {
            ...
            Vec3 headWorldPos = animatable.getPlayer().position().add(headPos.x, headPos.y, headPos.z);
            ...
            Vec3 body1WorldPos = animatable.getPlayer().position().add(body1Pos.x, body1Pos.y, body1Pos.z);
            Vec3 body2WorldPos = animatable.getPlayer().position().add(body2Pos.x, body2Pos.y, body2Pos.z);
            Vec3 tail1WorldPos = animatable.getPlayer().position().add(tail1Pos.x, tail1Pos.y, tail1Pos.z);
            Vec3 tail2WorldPos = animatable.getPlayer().position().add(tail2Pos.x, tail2Pos.y, tail2Pos.z);
